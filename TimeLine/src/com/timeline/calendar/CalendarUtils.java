...
public class CalendarUtils {
	...
	private static List<String> resultWeekDateStrs = new ArrayList<String>();
	private int row = 6;
	 ...
	 * @return 一周内所有天的日期字符串
	 */
	private static List<String> getNextWeekDateStr(List<String> dates) {
		List<String> dateStrs = new ArrayList<String>();
		SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd"); 
		String strDate = dates.get(dates.size() -1);
		Date date = null;
		try {
	 ...
	 * @return 一周内所有天的日期字符串
	 */
	private static List<String> getPreviousWeekDateStr(List<String> dates) {
		List<String> dateStrs = new ArrayList<String>();
		SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd"); 
		String strDate = dates.get(0);
		Date date = null;
		try {
			date = dateFormat.parse(strDate);
		} catch (ParseException e) {
	...
	public List<String> getSelectedWeek(int mPageNumber, List<String> dates) {
		resultWeekDateStrs = dates;
		if (mPageNumber > 500) {
			for (int i = 0; i < mPageNumber - 500; i++) {
				resultWeekDateStrs = getNextWeekDateStr(resultWeekDateStrs);
			}
		} else if (mPageNumber < 500) {
			for (int i = 0; i < 500 - mPageNumber; i++) {
				resultWeekDateStrs= getPreviousWeekDateStr(resultWeekDateStrs);
			}
		}
		return resultWeekDateStrs; 
	}
